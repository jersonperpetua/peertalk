...
@interface PTProtocol () {
  PTFrameTag nextFrameTag_;
  @public
  dispatch_queue_t queue_;
}
- (dispatch_data_t)createDispatchDataWithFrameOfType:(PTFrameType)type frameTag:(PTFrameTag)frameTag payload:(dispatch_data_t)payload;
@end
...
- (void)sendFrameOfType:(PTFrameType)frameType tag:(PTFrameTag)tag withPayload:(dispatch_data_t)payload overChannel:(dispatch_io_t)channel callback:(void(^)(NSError*))callback {
...
- (void)readFrameOverChannel:(dispatch_io_t)channel callback:(void(^)(NSError *error, PTFrameType frameType, PTFrameTag frameTag, uint32_t payloadSize))callback {
...
- (void)readFramesOverChannel:(dispatch_io_t)channel onFrame:(void(^)(NSError*, PTFrameType, PTFrameTag, uint32_t, dispatch_block_t))onFrame {
  [self readFrameOverChannel:channel callback:^(NSError *error, PTFrameType type, PTFrameTag tag, uint32_t payloadSize) {
